...
public class GUI extends javax.swing.JFrame {
	...
	public void shareFile(final int row) {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				String Hash = (String) model.getValueAt(row, 6);
				...
				if (share) {
					...
					model.setValueAt("SHARING", row, 7);
					// JOptionPane.showMessageDialog(null,
					// "Share file successful", null, 1);
				} else {
					// JOptionPane.showMessageDialog(null,
					// "Can't share this file", "Error", 0);
				}
			}
		}).start();
	...
	private void unShareFile() {
		int row1 = fileTable.getSelectedRow();
		if (row1 < 0) {
			return;
		}
		// DefaultTableModel model = (DefaultTableModel)
		// fileTable.getModel();
		final int row = fileTable.convertRowIndexToModel(row1);
		final int ID = (Integer) model.getValueAt(row, 0);
		model.setValueAt("SEEDED", row, 7);
	...
	private void shutdownThread(int ID) {
		...
		System.out.println("User wants to close thread processing file: "
				+ fileID);
	...
	private void limitRate(int row, int rate) {
		...
		System.out.print("User wants to limit file: " + ID + " rate to " + rate
				+ "kB\n");
	...
	public void drawTable() {
		...
		for (i = 0; i < v.size(); i++) {
			...
			Integer done = Long.valueOf(curSize * 100 / fileSize).intValue();
			if (fileStatus != null) {
			...
			Files f = new Files(Integer.valueOf(fileID), fileName,
					formatSize(fileSize), null, done, null, hash, fileStatus);
	...
	public String formatSize(long size) {
		if (size < 1024)
			...
			return (size * 100 / 1024) / 100.0 + " kB";
		else if (size < 1024 * 1024 * 1024)
			return (size * 100 / 1024 / 1024) / 100.0 + " MB";
		else
			return (size * 100 / 1024 / 1024 / 1024) / 100.0 + " GB";
	}
	 ...
	 * 
	 * 
	 * Vector<Integer> v = client.dataManager.getAllFile(); for (i = 0; i <
	 * v.size(); i++) { fileID = v.get(i); if
	 * (client.threadManager.getThread(fileID) != null) { curSize =
	 * client.threadManager.getThread(fileID).getcurSize();
	...
	private void popupSetup() {
		...
		id = fileTable.convertRowIndexToModel(id);
		String str = (String) model.getValueAt(id, 7);
	...
	private void deleteFile(int ID) {
		ID = fileTable.convertRowIndexToModel(ID);
		if ("SHARING".equals(model.getValueAt(ID, 7)))
			this.unShareFile();
		...
		/*
		 * ClientThread t = (ClientThread) model.getSwingWorker(ID); if (t !=
		 * null) { t.sendMsg("CLOSE @CODE: [fuckent]"); }
		 */
	...
	private void about() {
		...
		JOptionPane
				.showMessageDialog(
						null,
						"Assignment 1 - Computer Network 1\nFile Sharing Program\n\nAuthor: Group 7",
						"About", 1);
	}
	 ...
	 * 
	 * @Override public void run() { while (true) { updateCurSize(); try {
	...
	private void initComponents() {
			...
			public Component prepareRenderer(TableCellRenderer tcr, int row,
				...
				try {
					...
					if (isRowSelected(row)) {
						...
						c.setBackground((row % 2 == 1) ? evenColor
								: getBackground());
					}
					return c;
				} catch (Exception e) {
		...
		TableColumn column = fileTable.getColumnModel().getColumn(4);
		...
		layout.setVerticalGroup(layout.createParallelGroup(
				javax.swing.GroupLayout.Alignment.LEADING).addComponent(
				jScrollPane1, javax.swing.GroupLayout.DEFAULT_SIZE, 222,
				Short.MAX_VALUE));

		java.awt.Dimension screenSize = java.awt.Toolkit.getDefaultToolkit()
				.getScreenSize();
		setBounds((screenSize.width - 786) / 2, (screenSize.height - 272) / 2,
				786, 272);
	...
	private void fileTableKeyPressed(java.awt.event.KeyEvent evt) {// GEN-FIRST:event_fileTableKeyPressed
		...
		if (evt.getKeyCode() == 127) {
			this.deleteMenuActionPerformed(null);
		} else if (evt.getKeyCode() == 10 && evt.getModifiers() == 0) {
			this.downloadMenuActionPerformed(null);
			...
			row = fileTable.convertRowIndexToModel(row);
			if ("PAUSED".equals(model.getValueAt(row, 7)))
				this.resumeMenuActionPerformed(null);
			if ("DOWNLOADING".equals(model.getValueAt(row, 7)))
				this.pauseMenuActionPerformed(null);
			
		}

		else if (evt.getKeyCode() == 155) {
			this.seedMenuActionPerformed(null);
		} 
	...
	private void jMenuItem4ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_jMenuItem4ActionPerformed
		...
		String str = JOptionPane.showInputDialog(null, "Enter file ID: ",
				"Download file", 1);
	...
	private void limitRateMenuActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_limitRateMenuActionPerformed
		String limit = JOptionPane.showInputDialog(null,
				"Enter limited rate (kB/s) (-1 to unlimit): ", null, 1);
		int row = fileTable.getSelectedRow();
		row = fileTable.convertRowIndexToModel(row);
